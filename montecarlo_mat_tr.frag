...
layout(location=25) uniform float refractive_index;
...
vec3 random_path(in vec3 D, in vec3 O)
{
    ...
    for(int i=0; i<=NB_BOUNCES; ++i)
    {
		...
		bool isRefractedRay = random_float() > color.a; // On décide si on lance un rayon réfléchi ou réfracté, selon l'opacité de la surface

		if(!isRefractedRay) // Si rayon réfléchi
		{
			// Rayon diffus
		...
		else // Si rayon réfracté
		{
			O = P-BIAS*N; // il faut se décaler un peu dans la direction opposée de la normale pour eviter les auto-intersections
			D = refract(D, N, 1.f/refractive_index); // On réfracte le rayon selon l'indice de réfraction de l'air sur l'indice de réfraction de l'objet (1./refractive_index)
			traverse_all_bvh(O, D); // On lance le rayon pour sortir du volume
			...
			intersection_info(N, P); // Ici, la normale est en direction de l'extérieur !!!!
			mat = intersection_mat_info(); // On récupère le matériaux
			color = intersection_color_info(); // On récupère la couleur

			O = P+BIAS*N; // il faut se décaler un peu dans la direction de la normale pour eviter les auto-intersections
			D = refract(D, -N, refractive_index); // On réfracte de nouveau le rayon, cette fois ci selon l'indice de réfraction de l'objet sur l'indice de réfraction de l'air (refractive_index/1.)

			total += mat.b * rayLight; // On accumule dans la couleur finale seulement si on a intersecté une surface emissive
			rayLight = mix(rayLight,rayLight * color.rgb, color.a); // On accumule la couleur selon le coefficient d'opacité
		}
    }
